    ...
    <div style={{display:'flex', gap:8}}>
      {vals.map((ch, i) => (
        <input
          ...
          style={{
            width:42, height:48, textAlign:'center', fontSize:22,
            border:'1px solid #444', background:'#111', color:'#fff', borderRadius:8
          }}
        />
      ))}
    </div>
...
function RegisterPhone() {
  ...
  const validateForm = () => {
    ...
    // Phone: exactly 11 digits, starts with 09 (adjust to your rules)
  ...
  const startOtp = async () => {
    ...
    try {
      ...
      setStep('otp');
      setResendIn(45); // cooldown
      // start countdown
      const iv = setInterval(() => setResendIn(x => {
        if (x <= 1) { clearInterval(iv); return 0; }
        return x - 1;
      }), 1000);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (step === 'otp') {
      ...
      setIsLoading(true);
      try {
        // Let the backend verify OTP and create the user in Supabase (server-side)
        const resp = await fetch('/api/otp/verify', {
          ...
          body: JSON.stringify({
            ...
            profile: {
              ...
              // send password only if you plan to store a hash in Supabase (server must hash!)
        ...
        // data.user should be the Supabase user/profile returned by backend
        login({
          id: data.user.id,
      ...
      <div className="register-container">
        ...
        <form onSubmit={handleSubmit} className="register-form">
                ...
                <div className="form-group">
                  ...
                  <div className="input-wrapper">
                    ...
                    <input id="firstName" name="firstName" type="text"
                      placeholder={t('enterFirstName')} className="form-input"
                ...
                <div className="form-group">
                  ...
                  <div className="input-wrapper">
                    ...
                    <input id="lastName" name="lastName" type="text"
                      placeholder={t('enterLastName')} className="form-input"
              ...
              <div className="form-group">
                ...
                <div className="input-wrapper">
                  ...
                  <input id="phone" name="phone" type="tel" inputMode="numeric"
                    placeholder="مثلاً 09123456789" className="form-input"
                    value={formData.phone} onChange={handleInputChange}/>
                </div>
                {errors.phone && <p className="form-error">{errors.phone}</p>}
              </div>

              {/* Password / Confirm (optional in OTP flows, keep if you need 2FA later) */}
              <div className="form-group">
                <label htmlFor="password" className="form-label">{t('password')}</label>
                <div className="password-input">
                  <span className="input-icon">🔒</span>
                  <input id="password" name="password" type="password"
                    placeholder={t('createStrongPassword')} className="form-input"
              ...
              <div className="form-group">
                ...
                <div className="password-input">
                  ...
                  <input id="confirmPassword" name="confirmPassword" type="password"
                    placeholder={t('confirmPasswordPlaceholder')} className="form-input"
              ...
              <div className="checkbox-group">
                <div className="checkbox-item">
                  <input id="acceptTerms" name="acceptTerms" type="checkbox"
                    checked={formData.acceptTerms} onChange={handleInputChange} className="checkbox-input"/>
                  <label htmlFor="acceptTerms" className="checkbox-label">
                    {t('agreeToTerms')} <Link to="/terms" className="link">{t('termsOfService')}</Link>
                  </label>
                </div>
                {errors.acceptTerms && <p className="form-error checkbox-error">{errors.acceptTerms}</p>}

                <div className="checkbox-item">
                  <input id="acceptPrivacy" name="acceptPrivacy" type="checkbox"
                    checked={formData.acceptPrivacy} onChange={handleInputChange} className="checkbox-input"/>
                  <label htmlFor="acceptPrivacy" className="checkbox-label">
                    {t('agreeToPrivacy')} <Link to="/privacy" className="link">{t('privacyPolicy')}</Link>
                  </label>
                </div>
          ...
          <button type="submit" className="register-button" disabled={isLoading}>
            {isLoading ? '⏳' : (step === 'form' ? t('sendVerificationCode') || 'Send code' : t('verifyAndCreate') || 'Verify & Create')}
          </button>
        </form>

        <div className="register-links">
          <span>{t('alreadyHaveAccount')} </span>
          <Link to="/login" className="link">{t('signInLink')}</Link>
        </div>
      </div>
    </div>
  );
