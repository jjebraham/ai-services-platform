...
class OTPService {
  constructor() {
    ...
    console.log('[OTP Service] Initialized with proxy fix');
  ...
  // Create axios instance with improved proxy handling
  createAxiosInstance() {
    const config = {
    ...
    if (proxyInfo) {
      try {
        // Create HTTPS agent with relaxed SSL settings for proxy
        ...
        try {
          const proxyAgent = new HttpsProxyAgent(proxyInfo.proxyUrl, {
            rejectUnauthorized: false,
            secureProtocol: 'TLSv1_2_method'
          });
          config.httpsAgent = proxyAgent;
          console.log('[OTP Service] Using HttpsProxyAgent');
        } catch (proxyError) {
          console.warn('[OTP Service] HttpsProxyAgent failed, using direct proxy config:', proxyError.message);
        ...
        console.error('[OTP Service] Failed to configure proxy:', e.message);
