...
class OTPService {
  constructor() {
    ...
    console.log('[OTP Service] OTP_MOCK env var:', process.env.OTP_MOCK);
  ...
  getProxyConfig() {
    ...
    // Parse proxy URL - handle both formats
    let proxyMatch = proxyUrl.match(/http:\/\/(.*?):(.*?)@(.*?):(\d+)/);
    if (proxyMatch) {
      return {
        host: proxyMatch[3],
    ...
    // Try simple format without auth in URL
    proxyMatch = proxyUrl.match(/http:\/\/(.*?):(\d+)/);
    if (proxyMatch) {
      const config = {
        host: proxyMatch[1],
      ...
      // Add auth from environment variables if available
      const authUser = process.env.PROXY_AUTH_USER;
      const authPass = process.env.PROXY_AUTH_PASS;
